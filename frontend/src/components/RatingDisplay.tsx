  ...
  return (
    <div className="space-y-4">
      {consultantRatings.length > 0 && (
        <div className="border border-muted/20 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            <Badge variant="outline">Consultant Ratings</Badge>
      ...
      {customerRatings.length > 0 && (
        <div className="border border-muted/20 rounded-lg p-4">
          <div className="flex items-center gap-2 mb-3">
            <Badge variant="outline">Customer Ratings</Badge>
            <span className="text-sm text-muted-foreground">({customerRatings.length})</span>
          </div>
          
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-3">
            <div className="text-center">
              <div className="text-sm font-medium mb-1">Communication</div>
              {renderStars(Math.round(calculateAverage(customerRatings, 'communicationProfessionalism')))}
              <div className="text-xs text-muted-foreground">
                {calculateAverage(customerRatings, 'communicationProfessionalism').toFixed(1)}/5
              </div>
            </div>
            <div className="text-center">
              <div className="text-sm font-medium mb-1">Resolution</div>
              {renderStars(Math.round(calculateAverage(customerRatings, 'resolutionQuality')))}
              <div className="text-xs text-muted-foreground">
                {calculateAverage(customerRatings, 'resolutionQuality').toFixed(1)}/5
              </div>
            </div>
            <div className="text-center">
              <div className="text-sm font-medium mb-1">Response Time</div>
              {renderStars(Math.round(calculateAverage(customerRatings, 'responseTime')))}
              <div className="text-xs text-muted-foreground">
                {calculateAverage(customerRatings, 'responseTime').toFixed(1)}/5
              </div>
            </div>
          </div>
          
          {customerRatings.some(r => r.comments) && (
            <div className="space-y-2">
              <div className="flex items-center gap-1 text-sm font-medium">
                <MessageSquare className="w-3 h-3" />
                Comments
